...
set.seed(seed)
setwd(wd)
file_out = paste("beta", true_beta, "_gamma", true_gamma, "_case", true_case, "_probZ", prob_Z, "_opt.tab", sep="")
sink(file_out)
cat("X\tZ\n")
sink()
...
while (nsim <= NSIM) {
    ...
    best_k = 30
    ncase = sum(delta)
    phase2_id = c(indZ0[order_resi0[1:best_k]], indZ0[order_resi0[(nZ0-best_k+1):nZ0]], indZ1[order_resi1[1:(ncase-best_k)]], indZ1[order_resi1[(nZ1-(ncase-best_k)+1):nZ1]])
    mart.opt = martingale_residual[phase2_id]
    simZ.opt = simZ[phase2_id]
    best_var = (var(mart.opt[which(simZ.opt == 0)])*sum(simZ.opt==0)+var(mart.opt[which(simZ.opt == 1)])*sum(simZ.opt==1))/(2*ncase)
    for (k in 31:(ncase-30))
    {
        phase2_id = c(indZ0[order_resi0[1:k]], indZ0[order_resi0[(nZ0-k+1):nZ0]], indZ1[order_resi1[1:(ncase-k)]], indZ1[order_resi1[(nZ1-(ncase-k)+1):nZ1]])
        mart.opt = martingale_residual[phase2_id]
        simZ.opt = simZ[phase2_id]
        tmp_var = (var(mart.opt[which(simZ.opt == 0)])*sum(simZ.opt==0)+var(mart.opt[which(simZ.opt == 1)])*sum(simZ.opt==1))/(2*ncase)
        if (tmp_var > best_var) {
            best_k = k
            best_var = tmp_var
        }
