	...
	do
		...
		do
			...
			do
				...
				echo "srun R CMD BATCH --no-save --no-restore --slave \"--args $NSIM $case $gamma $njob $wd/res/$njob $seed $beta\" $wd/sim.R beta${beta}_gamma${gamma}_case${case}.Rout" >> tmp.out
				chmod 700 tmp.out
				# cat tmp.out
				sbatch tmp.out
				rm -rf tmp.out
