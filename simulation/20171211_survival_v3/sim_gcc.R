...
set.seed(seed)
setwd(wd)
file_out = paste("beta", true_beta, "_gamma", true_gamma, "_case", true_case, "_probZ", prob_Z, "_gcc.tab", sep="")
sink(file_out)
cat("X\tZ\n")
sink()
...
while (nsim <= NSIM) {
    ...
    phase2_id = c(sample(which(delta == 1), 200), sample(which(delta == 0), 200))
    dat1 = dat
    dat1[-phase2_id,"X"] = NA
