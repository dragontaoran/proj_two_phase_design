...
set.seed(seed)
setwd(wd)
file_out = paste("beta", true_beta, "_gamma", true_gamma, "_case", true_case, "_probZ", prob_Z, "_scc.tab", sep="")
sink(file_out)
cat("X\tZ\n")
sink()
...
while (nsim <= NSIM) {
    ...
    n10 = length(which(delta == 1 & simZ == 0))
    if (n10 > 100) {
        n10 = 100
    }
    n11 = length(which(delta == 1 & simZ == 1))
    if (n11 > 100) {
        n11 = 100
    }    
    phase2_id = c(sample(which(delta == 1 & simZ == 0), n10),
                  sample(which(delta == 1 & simZ == 1), n11),
                  sample(which(delta == 0 & simZ == 0), 200-n10),
                  sample(which(delta == 0 & simZ == 1), 200-n11))
    dat1 = dat
    dat1[-phase2_id,"X"] = NA
