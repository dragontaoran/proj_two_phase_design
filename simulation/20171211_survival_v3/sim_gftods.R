...
set.seed(seed)
setwd(wd)
file_out = paste("beta", true_beta, "_gamma", true_gamma, "_case", true_case, "_probZ", prob_Z, "_gftods.tab", sep="")
sink(file_out)
cat("X\tZ\n")
sink()
...
while (nsim <= NSIM) {
    ...
    #### general failure-time outcome-dependent sampling ######################################################################
    time_id = (201:N)[which(delta[201:N] == 1)]
    order_time = order(time[time_id])
    n_event = length(time_id)
    phase2_id = c(1:200, time_id[order_time[1:100]], time_id[order_time[(n_event-100+1):n_event]])
    dat1 = dat
    dat1[-phase2_id,"X"] = NA
