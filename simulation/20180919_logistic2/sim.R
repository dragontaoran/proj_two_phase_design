...
while (nsim <= NSIM) {
    ...
    if (min(table(simY, simZ)) <= 10) {
        ...
        phase2_id = c(sample(which(simY == 1), 200), sample(which(simY == 0), 200))
        ...
        phase2_id = c(sample(which(simY == 1 & simZ == 0), 100),
                      sample(which(simY == 1 & simZ == 1), 100),
                      sample(which(simY == 0 & simZ == 0), 100),
                      sample(which(simY == 0 & simZ == 1), 100))
        # table(simY[phase2_id], simZ[phase2_id])
        dat1 = dat
        dat1[-phase2_id,"X"] = NA
        ...
        start = max(k_init, 400-nZ1)
        end = min(400-k_init, nZ0)
