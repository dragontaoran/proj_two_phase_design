...
#### software options
wd="/nas/longleaf/home/taor/tmp/conXZ" 
NJOB="500"
tau_set=("0" "-0.7")
true_beta=("0" "0.3" "0.5")
true_gamma=("0" "0.5" "1")
...
do
	...
	do
		...
		do
			...
			do
				FN_slog="tau${tau}_beta${beta}_gamma${gamma}_rho${rho}.slog"
				FN_error="tau${tau}_beta${beta}_gamma${gamma}_rho${rho}.error"
				...
				echo "R CMD BATCH --no-save --no-restore --slave \"--args ${NSIM} ${tau} ${gamma} ${njob} ${wd}/res/${njob} ${seed} ${rho} ${beta}\" ${wd}/sim.R tau${tau}_beta${beta}_gamma${gamma}_rho${rho}.Rout" >> tmp.out
				chmod 700 tmp.out
				# cat tmp.out
				sbatch tmp.out
				rm -rf tmp.out
