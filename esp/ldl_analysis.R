...
FN.out = "/home/users/taor/Research/two_phase_GLM/design/esp/ldl_analysis.out"
...
bmi = scale(pheno$logbmi, TRUE, TRUE)
z = pheno$race
w = cbind(age, pheno$aric, pheno$cardia, pheno$chs, pheno$fhs, pheno$jhs, pheno$mesa, bmi)

my = mean(pheno$logldl)
sy = sd(pheno$logldl)
ma = mean(pheno$esp_age_baseline)
sa = sd(pheno$age)
mb = mean(pheno$logbmi)
sb = sd(pheno$logbmi)
...
## rds
resy = residuals(lm(logldl~race+esp_age_baseline+aric+cardia+chs+fhs+jhs+mesa+logbmi, data=pheno))
