...
{
    dat1 = dat
    phase2 = c(which(dat1$rel == 1), sample(which(dat1$rel == 0), 571))
    table(dat$instit[phase2], dat$rel[phase2])
    dat1$uh[-phase2] = NA
    res_cc = smle(Y="edrel", Delta="rel", X="uh", Z=c("luh","stage2","stage3","stage4","age"), 
                  Bspline_Z=c("bs1","bs2"), data=dat1, model="coxph", hn_scale=1)$coefficients
    if (sum(is.na(res_cc[,1]))+sum(is.na(res_cc[,2])) == 0) {
        cat(paste(res_cc[1,1], res_cc[1,2], 
                  res_cc[2,1], res_cc[2,2],
                  res_cc[3,1], res_cc[3,2],
                  res_cc[4,1], res_cc[4,2],
                  res_cc[5,1], res_cc[5,2],
                  res_cc[6,1], res_cc[6,2], sep="\t"))
        cat("\n")
        i = i+1
    }
